package ru.projects.methods.TASK_13_14.rynge_kyt;
...
public class RyngeKyt extends Method {
    ...
    public ArrayList<RyngeKytTable> createCells() {

        ArrayList<RyngeKytTable> forTable = new ArrayList<>();

        for (int i = 0; i < x.size(); i++) {
            forTable.add(new RyngeKytTable(i, h, x.get(i), y.get(i), z.get(i), delta_z.get(i), delta_y.get(i), istFunc(x.get(i)), Math.abs(istFunc(x.get(i)) - y.get(i))));
        }
